...
const AdminCommuniyGallery = () => {
  ...
  useEffect(() => {
    const getList = async () => {
      try {
        const { data } = await axios.get('http://localhost:8080/photogallery/list');
        setGalleryList(data);
      } catch (error) {
        console.error(error);
          ...
          {galleryList.map((event, index) => (
            <tr key={index}>
              ...
              {/* <td>
                <input
                  type="checkbox"
                  checked={selectedGalleryId === event.galleryId}
              ...
              <td>{event.title}</td>
              <td>{event.createdAt ? new Date(event.createdAt).toLocaleDateString() : '날짜 미정'}</td>
            </tr>
          ))}
        </tbody>
      </table>
